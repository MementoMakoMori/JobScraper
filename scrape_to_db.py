...
def ex_scrape(id_codes: list):
    with sync_playwright() as p:
        b = p.firefox.launch(headless=False, slow_mo=50)
        page = b.new_page()

        def block_requests(rt):
        ...
        def scrape(id_code):
            ...
            while True:
                try:
                    page.goto(f'https://www.indeed.com/viewjob?jk={id_code}', wait_until="load")
                    t1 = time.time()
                    while "Just a moment" in page.title():
                        time.sleep(0.1)
                        t2 = time.time()
                        if t2 - t1 >= 20:
                            print(f"Chunk {chunkn}: Load timeout on {id_code}, skipping.", flush=True)
                            return None
                except PlaywrightTimeoutError:
                    print(f"Chunk {chunkn}: Timeout error to {id_code}, attempting reload...", flush=True)
                    page.reload()
                    continue
                break
            if "Page Not Found" in page.title():
                print(f"Chunk {chunkn}: Job {id_code} no longer exists.", flush=True)
                return None
            occ = page.query_selector('.jobsearch-JobInfoHeader-title-container')
            if not occ:
                print(f"Chunk {chunkn}: Failed to find job content.", flush=True)
                return None
            else:
                return occ.inner_text()
        # while not occ:
        #     print(f"Job content not found for {id_code}. Attempt to wait...")
        #     time.sleep(3)
        #     occ = page.query_selector('.jobsearch-JobInfoHeader-title-container')
        #     occ.click()
        #     print("Job content not found. Attempt to reload...")
        #     page.goto('https://www.indeed.com/viewjob?jk=78be4895d4e664a2', wait_until="load")
        #     occ = page.query_selector('.jobsearch-JobInfoHeader-title-container')
        #     if not occ:
        #         print("Failed to find job content.")
        #         return None
        total = 0
        for each in id_codes:
            if scrape(each):
                total += 1
            count += 1
            if count % 34 == 0:
                ...
                page = b.new_page()
        b.close()
        return f"Chunk {chunkn}: returned {total} of {len(id_codes)} job posts."
